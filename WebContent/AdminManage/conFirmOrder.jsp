	...
	<div  class="table table-striped table-hover table-bordered">
     ...
     <tbody  id="tbody" > 
		...
		<tr >
			...
			<td style=''>${sorder.id}</td>
			<td class=" text-center">
				<input name="orderForm.remark" value="备注"></input>
			</td>
			<td>
...
<tr>
    ...
    <c:choose>
            ...
            <td style="width:50px">
            <c:choose>
                <c:when test="${sessionScope.order.lastPage}">
...
firm.onclick = function(){
     ...
     if(cks[i].checked) { //如果当前被选中
    	 var tr = cks[i].parentNode.parentNode;
         var temp = tr.children[8].innerHTML;
         console.log(temp);
         $.ajax({url:"confirmOrder.action?orderForm.id="+temp,async:false});
         }
	 }
}
